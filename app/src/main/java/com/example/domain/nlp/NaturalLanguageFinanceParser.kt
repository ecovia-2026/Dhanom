...
object NaturalLanguageFinanceParser {
    ...
    fun parseCommandWithContext(
        ...
        // 12b. Savings / emergency fund declarations: "I have ₹50,000 emergency fund",
        //      "savings of 2 lakh", "FD of 1 lakh". These are SAVINGS, never expenses.
        val savAmount = extractAmount(lower)
        if (savAmount > 0 && (lower.contains("emergency") || lower.contains("savings") ||
                lower.contains("fixed deposit") || lower.contains("saved") || lower.contains("fd ") ||
                lower.contains("deposit") || lower.contains("jama"))) {
            return ParsedFinanceCommand.AddGoalCommand(
                goal = GoalEntity(
                    title = "Emergency Fund",
                    targetAmount = savAmount,
                    currentAmount = savAmount,
                    categoryTag = "Security"
                ),
                confirmationMessage = "✅ Noted! ₹${String.format(Locale.US, "%,.0f", savAmount)} recorded as your Emergency Fund savings (not an expense)."
            )
        }
